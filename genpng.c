...
static image_t *gen_image(int width, int height)
{
    ...
    memset(img->buffer, 0, (unsigned long)(img->stride * img->height));
    int i;
...
static void blend_single(image_t * frame, ASS_Image *img)
{
    ...
    unsigned char *dst;
        int d = 4;
    src = img->bitmap;
    dst = frame->buffer + img->dst_y * frame->stride + img->dst_x * d;
    for (y = 0; y < img->h; ++y) {
        for (x = 0; x < img->w; ++x) {
            unsigned k = ((unsigned) src[x]) * opacity / 255;
            // possible endianness problems
            dst[x * d + 0] = (k * b + (255 - k) * dst[x * d + 0]) / 255;
            dst[x * d + 1] = (k * g + (255 - k) * dst[x * d + 1]) / 255;
            dst[x * d + 2] = (k * r + (255 - k) * dst[x * d + 2]) / 255;
            if (k) {
                dst[x*d+3] = 255;
            }
        }
...
int main(int argc, char *argv[])
{
  printf("%s %s\n", argv[3], argv[4]);
    ...
    if (argc < 6) {
        printf("usage: %s <image file> <subtitle file> <time> <width> <height>\n", argv[0]);
        exit(1);
    }

    const int frame_w = atoi(argv[4]);
    const int frame_h = atoi(argv[5]);
